...
app.get("/api/get", (req, res) => {
  ...
  // eslint-disable-next-line no-undef
  request = new Request(
    "SELECT [Valid_Pass], [License_Plate_Number],[License_Plate_State], [Color], [Make], [Model], [Confidence], [entry], [stamp] FROM [dbo].[Vehicle] FULL JOIN dbo.Vehicle_History ON Vehicle.License_Plate_Number = Vehicle_History.Plate_Number ORDER BY [stamp] DESC",
    function (err, rowCount) {
      if (err) {
        console.log(err);
      } else {
...
app.get("/api/getparkingold", (req, res) => {
  ...
  // eslint-disable-next-line no-undef
  request = new Request(
    "SELECT [Num_Spots], [Name], [Num_Used] FROM [master].[dbo].[Parking_lots]",
    function (err, rowCount) {
      if (err) {
        console.log(err);
      } else {
...
app.get("/api/getparking", (req, res) => {
  ...
  // eslint-disable-next-line no-undef
  request = new Request(
    "SELECT COUNT(*) FROM Vehicle FULL JOIN Vehicle_History ON Vehicle.License_Plate_Number = Vehicle_History.Plate_Number WHERE Vehicle_History.Stamp IN (SELECT MAX(Vehicle_History.Stamp) AS Stamp FROM Vehicle_History GROUP BY Vehicle_History.Plate_Number) AND Entry = '1'",
    function (err, rowCount) {
      if (err) {
        console.log(err);
      } else {
